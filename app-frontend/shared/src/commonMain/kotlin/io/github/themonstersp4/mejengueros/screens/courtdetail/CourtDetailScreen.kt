...
private fun DisponibilidadSection(
  ...
  Column(
      modifier =
          Modifier.fillMaxWidth().padding(horizontal = 20.dp, vertical = 20.dp).testTag("court_detail_disponibilidad_section"),
...
private fun SlotGrid(slots: List<CourtDetailSlot>) {
  FlowRow(
    ...
    slots.forEach { slot ->
      SlotChip(label = slot.displayTime, isTaken = false)
    }
  }
}
...
private fun ServiciosSection(services: List<String>) {
  Column(
      modifier =
          Modifier.fillMaxWidth().padding(horizontal = 20.dp, vertical = 20.dp).testTag("court_detail_servicios_section"),
      verticalArrangement = Arrangement.spacedBy(14.dp),
  ) {
    Text(
        text = "Servicios",
